...
| `make nfs-setup` | Install `nfs-kernel-server`, extract the built rootfs into `NFS_DIR`, and export it (`no_root_squash`) so the board can NFS-root mount it. Needs root (`NFS_SUDO=sudo`). Idempotent — preserves live edits unless re-extraction is forced with `make nfs-setup NFS_FORCE=1` (after a rebuild). |
...
| `LINUX_RT` | `0` | Build the realtime **PREEMPT_RT** kernel instead of the stock low-latency (`CONFIG_PREEMPT`) one. `1` makes the `meta-custom-bsp` `linux-adi` bbappend add a config fragment enabling `CONFIG_EXPERT` + `CONFIG_PREEMPT_RT` — PREEMPT_RT is mainlined in this BSP's Linux 6.12, so no `-rt` patch/recipe is needed. Changing it rebuilds the kernel; confirm on the board with `uname -a` (shows `PREEMPT_RT`). |
